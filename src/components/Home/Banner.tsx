...
const Banner = ({ isTop = false, info }: any) => {
  ...
  const baseDescriptionStyle =
    "flex flex-col item-center gap-18 max-w-[1200px] w-screen h-full mt-48 text-32 text-700 leading-45 text-center z-1 md:mt-84 md:text-40 md:leading-56 xl:mt-0 xl:justify-center xl:item-start xl:text-left";
    ...
    <div className={`${baseBannerStyle} `}>
      <div className={`${bannerStyle}`}>
        ...
        <div className={`${baseDescriptionStyle}`}>
          ...
          {buttonText && (
            <BaseButton
              className="mx-auto w-154 text-16 md:w-335 md:text-20 xl:mx-0"
              as="Link"
              href="/items"
            >
              {buttonText}
